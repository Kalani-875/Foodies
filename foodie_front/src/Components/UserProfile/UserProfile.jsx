...
const UserProfilePage = () => {
  ...
  const handleFollowButtonClick = () => {
    ...
    axios
      .post(`${config.baseUrl}/follow`, data)
      .then((value) => {
  ...
  const handleUnfollow = () => {
    axios
      .delete(`${config.baseUrl}/follow/${followId}`)
      .then((value) => {
        setFollowed(false);
        window.location.reload();
      })
  ...
  const setIsFollowedFromDb = () => {
    axios
      .get(`${config.baseUrl}/follow/byFollowedBy/${id}`)
      .then((value) => {
        ...
        axios
          .get(`${config.baseUrl}/follow/byFollowedTo/${id}`)
          .then((value) => {
            //  console.log(value.data[0]["id"])
            if (value.data.length > 0) {
              setFollowed(true);
